...
class BookSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        ...
        if user.is_authenticated:
            representation['is_favorite'] = Favorite.objects.filter(user=user, book=instance).exists()
...
class RatingSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if Rating.objects.filter(user=user, book=book).exists():
            raise ValidationError({'message': 'Вы уже оставляли рейтинг для этой книги'})
        return attrs
